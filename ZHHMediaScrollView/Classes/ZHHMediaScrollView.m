...
@property (nonatomic, weak) id<ZHHMediaScrollViewDelegate> userDelegate;

// 创建三个控制视图，用于滑动切换
@property (nonatomic, strong) ZHHMediaScrollViewCell *topCell; // 顶部视图
@property (nonatomic, strong) ZHHMediaScrollViewCell *ctrCell; // 中间视图
@property (nonatomic, strong) ZHHMediaScrollViewCell *btmCell; // 底部视图
...
// 当前显示的view
@property (nonatomic, weak) ZHHMediaScrollViewCell *currentCell;
...
// 当前正在更新的view
@property (nonatomic, weak) ZHHMediaScrollViewCell *updateCell;

// 处理view即将显示
@property (nonatomic, assign) CGFloat lastOffsetY;
@property (nonatomic, weak) ZHHMediaScrollViewCell *lastWillDisplayCell;
@property (nonatomic, weak) ZHHMediaScrollViewCell *lastEndDisplayCell;
...
// 移除cell
@property (nonatomic, weak) ZHHMediaScrollViewCell *willRemoveCell;
...
- (void)setDelegate:(id<ZHHMediaScrollViewDelegate>)delegate {
...
- (NSIndexPath *)indexPathForCell:(ZHHMediaScrollViewCell *)cell {
...
- (__kindof ZHHMediaScrollViewCell *)cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    NSInteger index = indexPath.row;
    if (index < 0 || index > self.totalCount - 1) return nil;
    ZHHMediaScrollViewCell *cell = nil;
...
- (void)registerClass:(Class)cellClass forCellReuseIdentifier:(NSString *)identifier {
    ...
    if (![cellClass isSubclassOfClass:ZHHMediaScrollViewCell.class]) {
        @throw [NSException exceptionWithName:NSInvalidArgumentException
                                       reason:@"must pass a class of kind ZHHMediaScrollViewCell"
                                     userInfo:nil];
    }
    if (identifier.length <= 0) {
        @throw [NSException exceptionWithName:NSInvalidArgumentException
...
- (Class)cellClassWithIdentifier:(NSString *)identifier {
    ...
    ZHHMediaScrollViewCell *nibCell = [self cellWithNib:nib identifier:identifier];
    // 放入重用池
...
- (ZHHMediaScrollViewCell *)cellWithNib:(UINib *)nib identifier:(NSString *)identifier {
    NSArray *views = [nib instantiateWithOwner:self options:nil];
    // 只能存在一个view且必须是ZHHMediaScrollViewCell或其子类
    if (views.count == 1 && [[views.firstObject class] isSubclassOfClass:ZHHMediaScrollViewCell.class]) {
        ZHHMediaScrollViewCell *nibCell = (ZHHMediaScrollViewCell *)views.firstObject;
...
- (ZHHMediaScrollViewCell *)dequeueReusableCellWithIdentifier:(NSString *)identifier {
    NSMutableSet *cells = self.reusableCells[identifier];
    
    ZHHMediaScrollViewCell *cell = cells.anyObject;
    if (cell) {
...
- (void)saveReusableCell:(ZHHMediaScrollViewCell *)cell {
    if (!cell) return;
...
- (ZHHMediaScrollViewCell *)cellForIndex:(NSInteger)index {
...
- (void)willDisplayCell:(ZHHMediaScrollViewCell *)cell forIndex:(NSInteger)index {
    if (index < 0 || index >= self.totalCount) return;
    if (!cell) return;
...
- (void)didEndDisplayingCell:(ZHHMediaScrollViewCell *)cell forIndex:(NSInteger)index {
    if (index < 0 || index >= self.totalCount) return;
    if (!cell) return;
...
- (void)didEndScrollingCell:(ZHHMediaScrollViewCell *)cell forIndex:(NSInteger)index {
    if (index < 0 || index >= self.totalCount) return;
    if (!cell) return;
...
- (void)didEndScrollingCell:(ZHHMediaScrollViewCell *)cell {
...
@interface ZHHMediaScrollView (UIScrollView)
